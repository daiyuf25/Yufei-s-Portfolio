...
For each round, program uncovers one piece at a time and then prompts the user(s) to make a guess.
...
def main():
    ...
    while len(puzzle_pieces_list) > 1 and guess != pictures[name_image_to_guess]:
        ...
        for i in range (1, players + 1):
            guess = input(f"It is player {i}'s turn. What do you think the image is? ")
            if guess == pictures[name_image_to_guess]:
            #if chat gpt thinks that the user's answer is valid, print that the user wins and show the entire image
                print(f"Congratulations! Player {i} wins.")
                image_to_guess.show()
                break
